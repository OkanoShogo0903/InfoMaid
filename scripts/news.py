...
class NewsData:
    ...
    def renew(self):
        """ 新しいニュースをQueueにいれて、その分だけ古いニュースが破棄される """
        # 新しいニュースを配列の先頭にいれる
        # RSSから各記事の情報を取得
...
class News:
    def __init__(self):
        ...
        threading.Thread(\
                target=self.initNewsList,\
                name="initNewsList",\
                daemon=True\
                ).start()

        # 一時間ごとにデータを更新するスレッドを登録
    ...
    def initNewsList(self):
        ...
        print("COMPLEAT!!")
        self.news_list = renew_data_list


    def renewNewsList(self):
        """ 古いニュースを破棄して、新しいデータを取得する """
